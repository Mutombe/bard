...
// Props from the server wrapper in page.tsx. `initialFeed` is the
// SSR-fetched first batch of 18 articles used both for ItemList JSON-LD
// and as SWR fallback data so Googlebot sees the headlines in the
// initial HTML (critical for ranking — a pure client-rendered homepage
// shipped an empty shell) and users get zero loading flash on first paint.
// Named `initialFeed` (not `initialArticles`) to avoid colliding with a
// local variable further down that derives the first-18 slice.
interface HomeViewProps {
  initialFeed?: { results: NewsArticle[]; count?: number; next?: string | null };
}

export default function HomeView({ initialFeed }: HomeViewProps = {}) {
  const [mounted, setMounted] = useState(false);
  const [extendedArticles, setExtendedArticles] = useState<NewsArticle[]>([]);
  const [loadingMore, setLoadingMore] = useState(false);
  const [hasLoadedMore, setHasLoadedMore] = useState(false);

  // FAST initial load — 18 articles for above-the-fold content. SSR feeds
  // the fallback so we skip the client fetch round-trip on first paint.
